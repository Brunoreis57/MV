...
import { X, Save, Edit2, Image, Phone, Clock, MapPin, Plus, Trash2, Facebook, Instagram, Mail } from 'lucide-react';
...
interface ServiceFormProps {
  onSubmit: (service: { name: string; description: string; price: string; category: 'estetica' | 'mecanica' }) => void;
  initialValues?: {
    name: string;
    description: string;
    price: string;
...
const ServiceForm: React.FC<ServiceFormProps> = ({ onSubmit, initialValues, onCancel }) => {
  ...
  const [category, setCategory] = useState<'estetica' | 'mecanica'>(initialValues?.category || 'estetica');
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <select
          value={category}
          onChange={(e) => setCategory(e.target.value as 'estetica' | 'mecanica')}
          className="w-full px-3 py-2 border rounded"
        >
...
export const AdminPanel: React.FC = () => {
  ...
  const tabs = [
    ...
    { id: 'services' as const, label: 'Serviços', icon: Plus },
                      ...
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          ...
                          <span className="inline-block px-2 py-1 text-sm rounded bg-gray-100 text-gray-700 mt-2">
                            {service.category === 'estetica' ? 'Estética' : 'Mecânica'}
                          </span>
                        </div>
